...
class cproc:
    ...
    #setting for multi thread option of qsub
    def multi_thread(self,thread_num,mpi_flag=False):
        if mpi_flag :
            self.thread_opt = "-pe mpi"
        else:
            self.thread_opt = "-pe def_slot"
        self.thread_opt = " ".join([self.thread_opt,str(thread_num)])
